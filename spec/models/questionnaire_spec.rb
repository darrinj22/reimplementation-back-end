...
describe Questionnaire, type: :model do
  ...
  let(:questionnaire1) { Questionnaire.new name: 'xyz', private: 0, max_question_score: 20, instructor_id: instructor_id }
  let(:questionnaire2) { Questionnaire.new name: 'pqr', private: 0, max_question_score: 10, instructor_id: instructor_id }
  ...
  describe '#instructor_id' do
    ...
    it 'returns the instructor id' do
      expect(questionnaire.instructor_id).to eq(instructor_id)
    end
  end
  ...
  describe '.copy_questionnaire_details' do
    ...
    it 'creates a copy of the questionnaire' do
      ...
      question1.save!
      question2.save!
      copied_questionnaire = Questionnaire.copy_questionnaire_details( { id: questionnaire.id, instructor_id: instructor_id})
    ...
    it 'creates a copy of all questions belonging to the original questionnaire' do
      ...
      question1.save!
      question2.save!
      copied_questionnaire = described_class.copy_questionnaire_details({ id: questionnaire.id, instructor_id: instructor_id })
  ...
  describe '#get_weighted_score' do
    ...
    it 'uses base symbol when used_in_round is nil' do
      ...
      allow(AssignmentQuestionnaire).to receive(:find_by).with(assignment_id: 42, questionnaire_id: questionnaire.id).and_return(aq)
  ...
  describe '#compute_weighted_score' do
    ...
    it 'returns 0 when average score is nil' do
      allow(AssignmentQuestionnaire).to receive(:find_by).with(assignment_id: 77).and_return(double('aq', questionnaire_weight: 25))
